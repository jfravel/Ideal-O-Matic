...
def run_case(PMFlag):
    ...
    if m.status in [2]:
        ...
        case_results.update({'P_delt':               tuple([m.getVarByName(f'delt[{s}]').x         for s in [0,1] ])})
        case_results.update({'P_DELT':                      m.getVarByName("DELTA").x })
        case_results.update({'P_eta':  tuple([tuple([tuple([m.getVarByName(f'eta[{c},{i},{s}]').x  for s in [0,1] ])  for i in [0,1] ]) for c in range(3) ]) })
        case_results.update({'P_nu':          tuple([tuple([m.getVarByName(f'nu[{c},{s}]').x       for s in [0,1] ])                    for c in range(2) ]) })
        case_results.update({'P_mu':                 tuple([m.getVarByName(f'mu[{c}]').x                                                for c in range(3) ]) })
        
        
        n = build_DIOM_SBM(case_results['x_LB'], case_results['x_UB'], case_results['x_PM'], max_runtime=MaxRuntime)
