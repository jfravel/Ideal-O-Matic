...
def build_DIOM_RU(
    ...
    m.addConstrs((                   eta[0,i,s] + eta[1,i,s] + eta[2,i,s] + eta[3,i,s] + zeta[s]  <=  3  for i in [0,1]  for s in [0,1] if PMFlag[s][i] == 1), name='Covr')
    m.addConstrs(( eta[0,i,s] + eta[0,(i+1)%2,s] + eta[2,(i+1)%2,s] + eta[3,(i+1)%2,s] + zeta[s]  <=  3  for i in [0,1]  for s in [0,1] if PMFlag[s][i] == 1), name='Covr')
    m.addConstrs(( eta[1,i,s] + eta[1,(i+1)%2,s] + eta[2,(i+1)%2,s] + eta[3,(i+1)%2,s] + zeta[s]  <=  3  for i in [0,1]  for s in [0,1] if PMFlag[s][i] == 1), name='Covr')
    m.addConstrs((               sum(eta[c,i,s] + eta[c,(i+1)%2,s]  for c in range(4)) + zeta[s]  <=  4  for i in [0,1]  for s in [0,1] if PMFlag[s][i] == 1), name='Covr')
    
    #Lemma 2.3
    m.addConstrs((               sum(eta[c,i,s] + eta[c,(i+1)%2,s]  for c in range(4)) + zeta[s]  <=  3  for i in [0,1]  for s in [0,1] if PMFlag[s][i] == 1 and PMFlag[s][(i+1)%2] == 1), name='Covr')
